...
exports.getHostAddress = async (id,prefix) => {
  try {
    let address = await PartyHosts.findOne({
      attributes : ['address','city','state_id','zip','phone','email','country_id'],
      where:{
        id : id
      },
      prefix
...
exports.getUserAddress = async(id,prefix) => {
  try {
    let address = await UserDetails.findOne({
      attributes : ['address','city','state_id','country_id','mobile','email'],
      where : {
        user_id : id
      },
      prefix
...
exports.insertParty = async (user_id,party,newAddress,hostAdd,userAdd,file,t,prefix) => {
  try {
