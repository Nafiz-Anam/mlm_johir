...
const partyUpload = require("../../middleware/web/partyUpload");
const multer = require("multer");
const { mlm_laravel } = require("../../models");
const Country = require("../../utils/web/allCountries");
const PartyServices = require("../../utils/web/partyServices");
const PartyGuests = db.partyGuests;
const { getAllCountries } = require("../../utils/web/allCountries");
    ...
    try {
        ...
        await partyUpload(req, res, async function (err) {
            if (err != undefined) {
                ...
                if (party.address) {
                    if (party.address == "host_address") {
                        if (postArr.host == "new") {
                            ...
                            var hostAddress =
                                await PartyServices.getHostAddress(
                                    party.host_id,
                                    prefix
                                );
                            if (hostAddress == false) {
                                ...
                                return res.status(422).json({
                                    ...
                                    error: {
                                        code: 1071,
                                        description: "Host address is empty",
                                    },
                                });
                            }
                        }
                        ...
                        if (
                            ...
                            return res.status(422).json({
                                ...
                                error: {
                                    code: 1072,
                                    description: "Owner address is empty",
                                },
                            });
                        }
                    } else {
                ...
                var file = req.file.file_name
                    ? `${process.env.image_url}tickets{req.file.file_name}`
                    : "";
