...
exports.getGenealogyTree = async (req, res) => {
  try {
    ...
    if (user_name != "") {
      ...
      let downlineCheck = await common.checkDownline(loggedId, userId, prefix);
      if (!downlineCheck) {
    ...
    let array = await reactTree(mlmPlan, rankStatus, userId, treeType, prefix);
    // console.log("**********************************", array);
    ...
    Object.entries(toolsTip).map(([key, value]) => {
      toolsArr[value.slug.replace(/-/g, "_")] = value.status ? true : false;
    });
    ...
    if (moduleStatus.ecom_status) {
      ...
      data = {
        ...data,
        store_url: await common.createEcomLink(userId, "register", prefix),
      };
    }
...
exports.getReferralMembers = async (req, res) => {
  try {
    ...
    offset = offset ? parseInt(offset) : 0;
    const moduleStatus = await modStatus.getModuleStatus(prefix);
    const totalLevel = await treeService.getMaxLevelSponsor(user_id, prefix);
    if (level != "all") {
      let levelValue =
        (await getUserTreeLevel(user_id, "sponsor_tree", prefix)) + level;
      ...
      if (binaryLeg.binary_leg != "any") {
        ...
        if (userLockedBinaryLeg != "any") {
          if (userId == value.user_id) {
            binaryDisabledLeg = userLockedBinaryLeg == "L" ? "R" : "L";
          } else {
...
exports.newGenealogyTree = async (req, res) => {
  try {
    ...
    let array = await reactTree1(mlmPlan, rankStatus, userId, treeType, prefix);
    ...
    result.forEach((element) => {
      result.forEach((element1) => {
        if (element.attributes.user_id == element1.attributes.father_id) {
      ...
      if (value.attributes.depth != 3) {
        if (moduleStatus.mlm_plan == "Binary") {
          if (value.children.length == 0) {
            ...
            } else if (value.children[0].attributes.position == "L") {
              let newData = {
                attributes: {
                  count: 0,
                  leg: "R",
