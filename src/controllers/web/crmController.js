...
exports.getFollowup = async (req, res) => {
    try {
        ...
        const { id } = req.user;
        let data = {
            followupsmissed: await crmService.getTodaysFollowups(
                id,
                "missed",
                prefix
            ),
            followupstoday: await crmService.getTodaysFollowups(
                id,
                "today",
                prefix
            ),
            followuprecent: await crmService.getTodaysFollowups(
                id,
                "recent",
                prefix
            ),
...
exports.addLeads = async (req, res) => {
    try {
        ...
        if (postArr["email_id"]) {
            let existingUser = await crmService.getUserIdByEmail(
                postArr["email_id"],
                prefix
            );
            if (existingUser > 0) {
                ...
                await Followup.create(
                    {
                        ...
                        image: req?.file?.filename
                            ? `${process.env.image_url}crm/${req.file.filename}`
                            : "",
                        followup_date: postArr.followup_date,
                    },
                    {
    ...
    try {
        ...
        for await (let [key, value] of Object.entries(leadHistory)) {
            ...
            historyArr[key] = {
                ...
                month_year_day: `${moment(value.created_at).format(
                    "D"
                )} ${moment(value.created_at).format("MMM")} ${moment(
                    value.created_at
                ).format("Y")}`,
                next_followup: `${moment(value.followup_date).format(
                    "D"
                )} ${moment(value.followup_date).format("MMM")} ${moment(
                    value.followup_date
                ).format("Y")}`,
