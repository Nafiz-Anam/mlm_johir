...
exports.getOrderHistory = async (req, res) => {
  try {
    ...
    for await (let [key, item] of Object.entries(Orders)) {
      ...
      let modelDetails = await Common.getOcModelNameAndPairValueFromUserId(
        id,
        prefix
      );
      ...
      let total_price = products.map(a => Number(a.price)*Number(a.quantity));
      orderDetails[key] = {
        date_added: new Date(item.date_added).toLocaleDateString(),
        order_id: item.order_id,
        order_id_with_prefix: item.order_id,
        firstname: item.shipping_firstname? item.shipping_firstname :"",
        ...
        shipping_firstname: item.shipping_firstname ? item.shipping_firstname : "",
        shipping_lastname: item.shipping_lastname ? item.shipping_lastname :"",
        shipping_address_1: item.shipping_address_1 ? item.shipping_address_1 :"",
        shipping_city: item.shipping_city ? item.shipping_city : "",
        shipping_zone: item.shipping_zone ? item.shipping_zone :"",
        shipping_country: item.shipping_country ? item.shipping_country:"",
        payment_firstname: item.payment_firstname ? item.payment_firstname :"",
        payment_lastname: item.payment_lastname ? item.payment_lastname: "",
        payment_address_1: item.payment_address_1 ? item.payment_address_1 :"",
        payment_city: item.payment_city ? item.payment_city :"",
        payment_country: item.payment_country ? item.payment_country :"",
        payment_zone: item.payment_zone ? item.payment_zone :"",
        price,
        total_price,
        products,
        order_total: await Common.getOcOrderTotal(item.order_id, prefix),
      };
    }
    return res.json({
      status: true,
