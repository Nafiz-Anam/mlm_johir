...
const WalletPayoutHistory = db.walletPayoutHistory;
            ...
            if (passCheck) {
                if (moduleStatus.multi_currency_status) {
                    ...
                    if (default_currency == null) {
                        ...
                        payout_amount = (
                            ...
                            parseFloat(defaultCurrencyValue)
                        ).toFixed(8);
                    } else {
                        let currencyValue = await common.getUserCurrencyValue(
                            default_currency,
                ...
                if (moduleStatus.kyc_status || !moduleStatus.kyc_status) {
                    ...
                    if (moduleStatus.kyc_status && !kycUpload.kyc_status) {
                        ...
                        let deductAmount =
                            Number(payout_amount) + Number(payoutFeeAmount);
                        const requestDate = Date("Y-m-d H:i:s");
                        let amounttoadd =
                            (Number(payout_amount) * Number(payoutFeeAmount)) /
                            100;
                        if (payoutConfig.fee_mode == "percentage") {
                            deductAmount =
                                Number(payout_amount) + Number(amounttoadd);
                        }
                        ...
                        if (
                            ...
                            try {
                                ...
                                const walletpayoutHst =
                                    await WalletPayoutHistory.create(
                                        {
                                            ref_id: 0,
                                            amount: Number(payout_amount),
                                ...
                                if (walletAddress.data.status) {
                                    ...
                                    const walletpayoutHst2 =
                                        await walletpayoutHst.update(
                                            {
                                                ref_id: PayoutId.id,
                                            },
                                            { transaction: t },
                                    ...
                                    return res.json({
                                        ...
                                        data: {
                                            message:
                                                "Payout request sent successfully",
                                        },
                                    });
                                } else {
                                    console.log(
...
exports.payoutRequestGet = async (req, res) => {
    try {
        ...
        const payoutReleaseAmount = await Payout.findAll({
            attributes: [
                [
                    Sequelize.fn("SUM", Sequelize.col("balance_amount")),
                    "amount",
                ],
            ],
            where: {
        ...
        let possiblePayoutAmt = userBalance.balance - payoutFeeAmount;
        ...
        if (payoutConfig.fee_mode == "percentage") {
            const amountToWIthdraw = (
                (Number(userBalance.balance) * Number(payoutFeeAmount)) /
                100
            ).toFixed(2);

            newAvailableMaxAmount =
                Number(userBalance.balance) - Number(amountToWIthdraw);
            console.log(newAvailableMaxAmount);
            var payout = `${payoutFeeAmount} % of Withdrawal Amount`;
        } else {
            console.log(
                ` user balance ${userBalance.balance}  amount to withdraw${payoutFeeAmount}`
            );
            newAvailableMaxAmount =
                Number(userBalance.balance) - Number(payoutFeeAmount);
...
exports.getApprovedPaidList = async (req, res) => {
    try {
        ...
        for await (let [key, value] of Object.entries(paidRequest)) {
            ...
            tableData[key] = {
                amount: value.amount,
                paid_date: moment(value.created_at).format(
                    "MMMM Do YYYY, h:mm:ss a"
                ),
...
async function getTotalAmountPendingRequest(userId, status, prefix) {
    ...
    const pendingAmt = await Payout.findAll({
        attributes: [
            [Sequelize.fn("SUM", Sequelize.col("balance_amount")), "sum"],
        ],
        where: whereStatement,
        raw: true,
        prefix,
