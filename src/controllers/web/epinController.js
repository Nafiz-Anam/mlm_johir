        ...
        if (action == "refund") {
            ...
            if (activePins != null) {
                try {
                    ...
                    var balance =
                        Number(userBalance.balance_amount) +
                        Number(pins.balance_amount);
...
exports.getEpinList = async (req, res) => {
    try {
        ...
        var { start, length, epins, amounts, status, direction, order } =
            req.query;
        let filters = {
        ...
        Object.entries(epinList).map(([key, value]) => {
            tableData[key] = {
                ...
                status:
                    value.expiry_date < new Date() ? "expired" : value.status,
                expiry_date: moment(value.expiry_date).format(
                    "MMMM Do YYYY, h:mm:ss a"
                ),
                refund:
    ...
    try {
        ...
        Object.entries(transferDetails).map(([key, value]) => {
            tableData[key] = {
                ...
                transferred_date: moment(value.date).format(
                    "MMMM Do YYYY, h:mm:ss a"
                ),
