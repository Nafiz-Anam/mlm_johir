  ...
  try {
    ...
    if (registrationFee.reg_amount >= 0 && moduleStatus.product_status) {
      ...
      Object.entries(paymentDetails).map(([key, value]) => {
        let icon, code, title;
        if (value.status && value.name != "E-pin" && value.name != "E-wallet") {
          switch (value.name) {
            case "Paypal":
              icon = "fa fa-paypal";
              title = "paypal_status";
    ...
    let paymentInfo = {
      ...
      fields: paymentField.filter((value) => Object.keys(value).length !== 0),
  ...
  try {
    if (user) {
      ...
      if (usrDetails) {
        ...
        if (amtFlag) {
          var transactionPass =
            process.env.DEMO_STATUS == "yes" ? "12345678" : Str.random(8);
