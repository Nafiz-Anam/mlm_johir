...
const {
  getLoggedUserId,
  getDefaultAddress,
} = require("../../utils/app/getLoggedUserId");
...
const { getAllLanguage } = require("../../utils/app/allLanguages");
...
const PurchaseServices = require("../../utils/app/purchaseServices");
...
exports.repurchaseSubmit = async (req, res) => {
  ...
  try {
    ...
    const activeRepurchasePaymentMethod = await paymentGatewayConfig.findAll({
      where: {
        status: 1,
        repurchase: 1,
      },
    ...
    if (postArray.payment_method === "purchase_wallet") {
      ...
      paymentType = await Common.getPaymentMethodId("purchase-wallet", prefix);
    ...
    if (postArray.payment_method === "ewallet") {
      ...
      let eWalletBalanceAmount = await PurchaseServices.getPurchaseWalletAmount(
        id,
        prefix
      );
    ...
    if (postArray.payment_method === "freejoin") {
      ...
      paymentType = await Common.getPaymentMethodId("free-joining", prefix);
      if (paymentType == false) {
        let response = await errorMessage({ code: 406 });
        return res.json(response);
      }
    ...
    if (postArray.payment_method === "banktransfer") {
      ...
      paymentType = await Common.getPaymentMethodId("bank-transfer", prefix);
      if (paymentType == false) {
        let response = await errorMessage({ code: 406 });
        return res.json(response);
      }
    ...
    if (isPwalletOk) {
      ...
      let insertHistory = await PurchaseServices.insertPurchasewalletHistory(
        id,
        id,
        amount,
        amount,
      ...
      if (insertHistory == false) {
        ...
        let deductBalance = await PurchaseServices.deductFromPurchaseWallet(
          id,
          amount,
          prefix,
          t
  ...
  try {
    ...
    let purchaseReport = await Order.findAll({
      ...
      where: {
        ...
        order_date: {
          [Op.between]: [
            (fromDate = moment(start_date).format("YYYY-MM-DD 00:00:00")),
