...
const donationServices = require("../../utils/app/donationServices");
...
exports.donationView = async (req, res) => {
    try {
        ...
        const { id } = req.user;
        const { donation_type } = await DonationConfiguration.findOne({
            prefix,
        });
        const moduleStatus = await modStatus.getModuleStatus(prefix);
        if (
            donation_type == "automatic" ||
...
exports.donate = async (req, res) => {
    ...
    try {
        ...
        if (currentLevel <= 3) {
            ...
            let transactionDetails = await Common.createUniqueTransaction(
                prefix,
                t
            );
