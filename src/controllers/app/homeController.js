...
const Lang = require("../../utils/app/allLanguages");
const userId = require("../../utils/app/getLoggedUserId");
...
const { successMessage } = require("../../utils/app/response");
...
const MailServices = require("../../utils/app/mailServices");
...
const { liveValueUpdates } = require("../../utils/app/liveValueServices");
  ...
  try {
    ...
    if (ModuleStatus.lang_status) {
      if (userLangDetails.default_lang) {
        let objIndex = Languages.findIndex((obj) => obj.default === true);
    ...
    const company_info = {
      ...
      favicon: companyDetailsResult.favicon ? companyDetailsResult.favicon : "",
      ...
      fb_link: companyDetailsResult.fb_link ? companyDetailsResult.fb_link : "",
...
exports.getTileFilter = async (req, res) => {
  ...
  switch (range) {
    ...
    case "thisWeek": // current date of week
      var currentWeekDay = date.getDay();
      var lessDays = currentWeekDay == 0 ? 6 : currentWeekDay - 1;
      fromDate = new Date(new Date(date).setDate(date.getDate() - lessDays));
      toDate = new Date(
        new Date(fromDate).setDate(fromDate.getDate() + 6)
      ).setHours(23, 59, 59);
      break;
...
exports.getNotifications = async (req, res) => {
  try {
    ...
    let packageNotification = await common.getMaxCommissionAlert(id, prefix);
    let alertMail;
    ...
    return res.json({
      ...
      data: {
        ...
        notification_details: {
          ...
          commission_count: packageNotification.percentage >= 90 ? 1 : 0,
        },
