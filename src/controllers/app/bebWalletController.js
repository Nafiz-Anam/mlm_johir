...
exports.getEwalletStatementTable = async (req, res) => {
  ...
  try {
    ...
    Object.entries(statement).map(([key, value]) => {
      ...
      if (value.type === "debit" && value.purchase_wallet != "fund_transfer") {
        balance =
          Number(balance) -
          Number(value.amount) -
      ...
      if (
        ...
        console.log("-------", balance, value.amount, value.purchase_wallet);
    ...
    if (totalRegAmount <= Number(fromUserBalance.balance_amount)) {
      ...
      if (checkPass) {
        ...
        const totalFromUserBal =
          Number(fromUserBalance.balance_amount) - Number(totalRegAmount);
    ...
    if (totalRegAmount <= Number(cashWalletBalanceDetails.purchase_wallet)) {
      ...
      if (checkPass) {
        ...
        const latestProductWallet =
          Number(cashWalletBalanceDetails.balance_amount) + Number(amount);
