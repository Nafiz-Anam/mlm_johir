...
} = require("../../utils/app/allLanguages");
const {
  getAllCurrencies,
  getAllCurrenciesWithoutPrefix,
} = require("../../utils/app/allCurrency");
const {
  getAllCountries,
  getAllCountriesWithoutPrefix,
} = require("../../utils/app/allCountries");
const uploadFile = require("../../middleware/app/bankUpload");
const { bankReceiptUnpproved } = require("../../utils/app/uploadServices");
...
const { approveRegister } = require("../../controllers/app/signupController");
...
exports.checkToken = async (req, res) => {
  try {
    ...
    if (decoded.user_type == "user") {
      const accessTokenFromDB = await Common.getAccessToken(decoded.id, prefix);
...
exports.getAppInfo = async (req, res) => {
  try {
    ...
    console.log(
      "==============================app info call from mobil=================="
    );
    let languages = [],
      currencies = [],
      companyDetailsResult = [];
    // if (prefix == process.env.default_api_key) {
    //   languages = await getAllLanguageWithoutPrefix();
    //   currencies = await getAllCurrenciesWithoutPrefix();
    // } else {
  ...
  try {
    ...
    if (updateEnroll) {
      ...
      if (pendingTransactionData && walletAddressData) {
        for (let [key, value] of Object.entries(pendingTransactionData)) {
        ...
        let latestBalance = walletAddressData?.balance
          ? Number(
              Number(walletAddressData?.balance) - Number(registrationAmount)
            )
          : 0;
        ...
        if (walletAddressDetails) {
          const updateWalletAddressDetails = await walletAddressDetails.update(
            {
              user_id: updatedId.updated_id,
            },
            { transaction: t },
        ...
        let newWalletHistoryEntry = await WalletHistories.create(
          {
            ...
            date_added: Date.now(),
            transaction_id:
              "registertransaction" + Math.floor(Math.random() * 10000),
        ...
        if (!newWalletHistoryEntry) {
          await t.rollback();
          let response = await errorMessage({ code: 1075 });
          return res.status(500).json(response);
        }
        await t.commit();
        let response = await successMessage({ value: updatedId.updated_id });
        return res.status(200).json(response);
      }
    } else {
      await t.rollback();
