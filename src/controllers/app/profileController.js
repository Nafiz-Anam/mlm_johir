...
const profileService = require("../../utils/app/profileService");
const UploadServices = require("../../utils/app/uploadServices");
...
const uploadFile = require("../../middleware/app/profilePicUpload");
const multipleUpload = require("../../middleware/app/uploadMultipleFile");
...
exports.updatePersonalDetails = async (req, res) => {
  ...
  try {
    ...
    if (ecomStatus) {
      const customerId = await CommonServices.getEcomCustomerRefId(id, prefix);
...
exports.updateSettingsDetails = async (req, res) => {
  ...
  const { language, currency, binaryLegSettings, googleAuthStatus } = req.body;
      ...
      if (moduleStatus.ecom_status) {
        ...
        if (!ocCustomer) {
          return res.status(400).send({
            status: false,
            message: "Oc_user not found for default language update",
          });
        }
        await ocCustomer.update(
          {
  ...
  try {
        ...
        include: [
          {
            ...
            model: pack,
            as: "package",
            attributes: ["id", "name", "product_id", "price", "pair_value"],
          },
        ],
        prefix,
      });
      var upgradablePackList = await CommonServices.getUpgradablePackageList(
        currentPack,
        prefix
      );
    ...
    res.status(200).json({
      ...
      data: {
        ...
        upgrade_list: newUpgradablePackageList ? newUpgradablePackageList : [],
      },
    });
  } catch (err) {
  ...
  try {
    ...
    await uploadFile(req, res, async function (err) {
      ...
      if (err != undefined) {
        ...
        console.log("====================================sucess");
        let response = await successMessage({
          value: `File uploaded succesfully ${req.file.originalname}`,
        });
    ...
    await multipleUpload(req, res, async function (err) {
      if (err != undefined) {
        ...
        if (req?.files?.length > 0) {
          ...
          if (uploadCount["upload_count"] >= uploadConfig["upload_config"]) {
            console.log(
              "==========================err in count ",
          ...
          if (uploadKycDocs != null) {
            ...
            return res.json({
              status: false,
              error: { code: 1041, description: "ID Already Exists" },
            });
          }
          console.log(
            "=================upload data =======================",
...
exports.getKycDetails = async (req, res) => {
  try {
    ...
    let documentsData = [];
    for await (let [key, value] of Object.entries(documentResult)) {
      documentsData[key] = {
        id: value.id,
        file_name: [await CommonServices.convertToIpUrl(value.file_name)],
        status:
          value.status == 1
            ? "approved"
            : value.status == 2
...
exports.subscriptionDetails = async (req, res) => {
  ...
  try {
    ...
    if (subscriptionData.product_validity == null) {
      ...
      validityDate = subscriptionData.product_validity.toLocaleDateString();
    }
    let data = {
...
exports.subscription = async (req, res) => {
  ...
  try {
    ...
    if (paymentStatus) {
      let packageupgraderesult = await profileService.packageValidityUpgrade(
        postArr.product_id,
        purchase,
        false,
        t,
...
exports.forgetTransactionPassword = async (req, res) => {
  try {
    ...
    if (result) {
      let response = await successMessage({
        message: "forgot transaction password reset mail sended successfully",
      });
      return res.json(response);
    }
  } catch (err) {
